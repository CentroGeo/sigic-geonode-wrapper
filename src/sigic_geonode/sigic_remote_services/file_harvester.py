...
PLACEHOLDER_SPATIAL_EXTENT = Polygon(
...
class FileHarvester(BaseHarvesterWorker):
    ...
    def update_geonode_resource(
        self, harvested_info: FileDataInfo, harvestable_resource: HarvestableResource
    ):
        ...
        if geonode_resource is None:
            ...
            geonode_resource = self._create_new_geonode_resource(
                geonode_resource_type, defaults
            )
            ...
            geonode_resource = self._update_existing_geonode_resource(
                geonode_resource, defaults
            )
        ...
        self.finalize_resource_update(
            geonode_resource, harvested_info, harvestable_resource
        )

    def get_geonode_resource_defaults(
        self,
        harvested_resource_info: FileDataInfo,
        ...
        defaults = {
            ...
            "supplemental_information": (
                harvested_resource_info.resource_descriptor.identification.supplemental_information
            ),
        ...
        defaults[
            "name"
        ] = harvested_resource_info.resource_descriptor.identification.name
        defaults.update(
            harvested_resource_info.resource_descriptor.additional_parameters
        )
        return defaults
...
def download_to_geonode(url: str, target_name: str, ext: str):
    ...
    with open(fn, mode="rb") as file:
        uploaded_file = UploadedFile(
            file, path.name, content_type=content_type, size=file_size, charset=charset
        )
        yield uploaded_file
