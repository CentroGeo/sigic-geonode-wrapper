...
router.register(r"api/v2/auth/user-group-roles", UserGroupRoleViewSet, basename="sigic-user-group-roles")
router.register(r"api/v2/auth/user", UserGroupRoleByUserViewSet, basename="sigic-user-group-roles-by-user")

urlpatterns = [
    path("api/v2/auth/group-roles/", MyGroupRolesView.as_view(), name="my-group-roles"),
    path("api/v2/auth/user/<str:email>/group-roles/", UserGroupRolesByEmailView.as_view(), name="user-group-roles-by-email"),
    path("api/v2/auth/user/invitation", UserInvitationCreateView.as_view(), name="invitation-create"),
    path("api/v2/auth/user/invitation/accept/<str:token>/", UserInvitationAcceptView.as_view(), name="invitation-accept"),
    path("api/v2/auth/user/invitation/callback/", UserInvitationCallbackView.as_view(), name="invitation-callback"),
